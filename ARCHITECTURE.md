...
**ADR-027: No explicit feature selection in either WP or QB EPA**

The WP model uses 7 hand-picked situational features — all known strong signals — so feature selection adds no value. The QB EPA model was initially designed with a `SelectFromModel` step between the preprocessor and XGBoost to handle sparse NGS columns (`time_to_throw`, `intended_air_yards`, `defense_coverage_type`). In practice, `threshold="mean"` collapsed the entire post-OHE feature set down to a single selected feature because one dominant feature (e.g. `score_differential`) pulled the mean importance threshold above every other column. The result was R2 of ~0.08 — the model was predicting from one variable. The selector was removed. XGBoost handles low-signal and sparse features natively: uninformative features simply do not get split on during tree construction, and the regularization parameters (`max_depth`, `subsample`) already control overfitting. Permutation importance is still logged as an artifact to surface which features actually contributed.
